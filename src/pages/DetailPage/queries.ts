...
export const fetchDetail = async <T>(id = '', type: 'movie' | 'tv'): Promise<T> =>
  fetcher(`${BASE_URL}/${type}/${id}?api_key=${process.env.REACT_APP_DB_API}&language=en-US`)
...
export const fetchVideos = async (id = '', type: 'movie' | 'tv'): Promise<Trailers> =>
  fetcher(`${BASE_URL}/${type}/${id}/videos?api_key=${process.env.REACT_APP_DB_API}&language=en-US`)
