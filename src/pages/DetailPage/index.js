...
const DetailPage = ({ type }) => {
  ...
  const rightNode = (
    <>
      <Typography className={classes.rankTop} variant="h4">{rank_average}</Typography>
    ...
    <>
      ...
      <div className={classes.rankStars}>
        <StarRatings
          ...
          starSpacing="5px"
        />
        <Typography className={classes.rank} variant="h3">{rank_average}</Typography> 
        <Typography className={classes.rankSecondary} variant="h5">{rank_count}</Typography>        
      </div>
    </>
  );
