...
export const getPopularMovies = async (): Promise<MovieListResponse> =>
  fetcher(`${BASE_URL}/movie/popular?api_key=${process.env.REACT_APP_DB_API}&language=en-US&page=1`)

export const getPopularTVSeries = async (): Promise<TVSeriesResponse> =>
  fetcher(`${BASE_URL}/tv/popular?api_key=${process.env.REACT_APP_DB_API}&language=en-US&page=1`)
